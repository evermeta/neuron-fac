...
describe("types.compareTypeSignature", function () {
    it("compares two type signatures, return true if equivalent", function () {
        ...
        testsOK.forEach(
            (equation) =>
                expect(types.compareTypeSignature(equation.l, equation.r)).to.be.true
        );
        testsNotOK.forEach(
            (equation) =>
                expect(types.compareTypeSignature(equation.l, equation.r)).to.be.false
        );
    });
});
...
describe("Type::addElement", function () {
    ...
    it(
        "the type function returns a type resolution " +
            "when called with the appropriate arg",
        function () {
            let COLOR = types.add({ typeName: "COLOR" });
            let COLORTRANSFORM = types.add({
                typeName: "COLORTRANS",
                typeSignature: [COLOR, COLOR],
            });
