...
const cpy = (obj: Record<string, unknown>): Record<string, unknown> => JSON.parse(JSON.stringify(obj)); 
...
export class DataPoint implements SetElement{
    ...
    public isSame(x: DataPointValue) :boolean {
        if(x instanceof DataPoint) return JSON.stringify(this.value) === JSON.stringify(x.value); 
